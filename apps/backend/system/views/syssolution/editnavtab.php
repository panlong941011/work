<div class="modal-header">
    ...
    <h4 class="modal-title"><?=Yii::t('app', '编辑菜单')?></h4>
</div>
	...
	<div class="row">
    	<div class="col-md-12">
            <div class="form-group">
                ...
                <input type="text" id="sName" class="form-control" value="<?=$tab->sName?>" placeholder="<?=Yii::t('app', '请输入菜单名称')?>">
            </div>
        </div>
    </div>
	<div class="row">
                ...
                <div class="portlet-body">
                    ...
                    <div class="dd" id="nestable_list_2" style="height:330px; overflow:auto">
                        ...
                        <ol class="dd-list">
                            ...
                            <li class="dd-item" data-id="<?=$item->navitem->ID?>">
                                <div class="dd-handle"> <?=$item->navitem->sName?> </div>
                            </li>
                            <? } ?>
                        </ol>
                        <? } else { ?>
...
function ok()
{
	...
	$.post(
		sHomeUrl+'/system/syssolution/editnavtabsave?ID=<?=$_GET['ID']?>',
		...
		function(data)
		{
			...
			if (ret.bSuccess) {
				location.href="<?=Yii::$app->homeUrl?>/<?=strtolower($this->context->sObjectName)?>/view?ID=<?=$tab->SolutionID?>";
			} else {
				error(ret.sMsg);
			}
		}
