function viewOrgChart()
{
    $.get
    (
        '<?=Yii::$app->homeUrl?>/<?=strtolower($this->context->sysObject->sObjectName)?>/viewchart',
        function(data)
        {
            var modal = openModal(data, $(window).width() * 0.9, $(window).height() * 0.8);
        }
    );
...
function editOperator(listTable)
{
	...
	parent.addTab('编辑角色操作权限', sHomeUrl+'/system/sysrole/editoperator?ID='+ID);
}
