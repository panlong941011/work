...
if ($this->context->action->id == 'view') {
	...
	foreach ($arrPic as $sLinkFieldValue) {
		?>
        <a target="_blank"
                href="<?= stristr($sLinkFieldValue, "://") ? $sLinkFieldValue : Yii::$app->params['sUploadUrl'] . "/" . $sLinkFieldValue ?>"><img <?= $field->attr['lImageWidth'] ? "width='" . $field->attr['lImageWidth'] . "'" : "width='127'" ?> <?= $field->attr['lImageHeight'] ? "height='" . $field->attr['lImageHeight'] . "'" : "height='127'" ?>
                    src="<?= stristr($sLinkFieldValue, "://") ? $sLinkFieldValue : Yii::$app->params['sUploadUrl'] . "/" . $sLinkFieldValue ?>"
            /></a>
    ...
    <input type="hidden" class="form-control" sDataType="<?= $field->sDataType ?>" sFieldAs="<?= $field->sFieldAs ?>"
            placeholder="" value="111" name="arrObjectData[<?= $field->sObjectName ?>][<?= $field->sFieldAs ?>]"
