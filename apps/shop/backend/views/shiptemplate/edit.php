    ...
    <div id="content">
        <div id="postage-tpl">
            <div class="setting">
                ...
                <form method="post" id="J_TPLForm" action="/shop/shiptemplate/edit?ID=<?= $_GET['ID'] ?>">
                    <? } else { ?>
                    <form method="post" id="J_TPLForm" action="/shop/shiptemplate/new">
                        <? } ?>
                        ...
                        <ul>
                            ...
                            <li class="form-elem">
                                ...
                                <input type="text" name="sName" value="<?= $ShipTemplate['sName'] ?>"
                                       class="input-text input-address" autocomplete="off" id="J_TemplateTitle"/>
                            </li>
    					...
    					<input type="hidden" id="J_FareTimeResult" name="consignDate" value="">
                                ...
                                <span>如实设定宝贝的发货时间，不仅可避免发货咨询和纠纷，还能促进成交！<a
                            ...
                            <li class="form-elem J_Freight" hidden>
                                ...
                                <span>
					    					<input type="radio" name="bearFreight" checked value="0"
                                                   id="J_buyerBearFre"/>
    					...
    					<label for="J_sellerBearFre">卖家承担运费</label>
                            ...
                            <li class="form-elem calc-method">
                                ...
                                <span>
						<input type="hidden" name="valuation" class="J_CalcRule"
                               value="<?= $ShipTemplate['valuationValue'] ?>"/>
						<label><?= $ShipTemplate['valuationName'] ?></label>
					</span>
                            </li>
                            <!--  区域限售  -->
                                ...
                                <table class="table">
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <a href="#" class="acc_popup edit J_Edit" title="编辑运送区域"
                                               data-areas="">编辑</a>
                                        ...
                                        <td>
                                            <select class="J_ChageContion">
                                                <option value="0">件数111</option>
                                                <option value="0">金额</option>
                                                <option value="0">件数+金额</option>
                                            </select>
                        ...
                        <input type="hidden" name="deliveryAddress" id="J_address"
                               value='<?= htmlspecialchars_decode($ShipTemplate['sDeliveryAddressJson']) ?>'/>
...
<script id="J_AreaTemplate" type="text/template">
	...
	<span class="gareas"><input type="checkbox" value="{{code}}" id="J_Province_{{code}}" class="J_Province"/>
		<label for="J_Province_{{code}}">{{title}}</label><span class="check_num"></span><img class="trigger"
                                                                                              src="//gtd.alicdn.com/tps/i1/T1XZCWXd8iXXXXXXXX-8-8.gif"></span>
                    <div class="citys">
                        {{#citys}}
                        <span class="areas"><input type="checkbox" value="{{code}}" id="J_City_{{code}}"
                                                   class="J_City"/>
...
<script id="J_RuleTemplate" type="text/template">
    <div class="entity">
        <div class="default J_DefaultSet">
            默认运费：<input type="text" name="{{delivery}}_start" value="{{start}}" data-field="start"
                        class="input-text {{#starterror}}input-error {{/starterror}}" autocomplete="off" maxlength="6"
                        aria-label="默认运费{{typeName}}数"/> {{typeName}}内，
            <input type="text" class="j_sellerBearFrePrice" value="0.00" disabled="disabled"/><input type="text"
                                                                                                     data-field="postage"
                                                                                                     name="{{delivery}}_postage"
                                                                                                     value="{{postage}}"
                                                                                                     class="input-text {{#postageerror}}input-error {{/postageerror}}"
                                                                                                     autocomplete="off"
                                                                                                     maxlength="6"
                                                                                                     aria-label="默认运费价格"/>
            元，
            每增加 <input type="text" name="{{delivery}}_plus" data-field="plus" value="{{plus}}"
                       class="input-text {{#pluserror}}input-error {{/pluserror}}" autocomplete="off" maxlength="6"
                       aria-label="每加{{typeName}}"/> {{typeName}}，
            增加运费 <input type="text" class="j_sellerBearFrePrice" value="0.00" disabled="disabled"/><input type="text"
                                                                                                          name="{{delivery}}_postageplus"
                                                                                                          data-field="postageplus"
                                                                                                          value="{{postageplus}}"
                                                                                                          class="input-text {{#postagepluserror}}input-error {{/postagepluserror}}"
                                                                                                          autocomplete="off"
                                                                                                          maxlength="6"
                                                                                                          aria-label="加{{typeName}}运费"/>元
            {{#global}}, <br/>全国时效：<input type="text" id="J_GlobalWayDay" name="{{delivery}}_wayDay" value="{{wayDay}}"
                                          data-field="wayDay"
                                          class="input-text {{#wayDayerror}}input-error {{/wayDayerror}}"
                                          autocomplete="off" maxlength="6" aria-label="全国时效"/> 天{{/global}}
            <div class="J_DefaultMessage">{{#message}}<span class="msg J_Message"><span
                            class="error">{{.}}</span></span>{{/message}}
            </div>
        </div>
        {{#withSpecial}}
        <div class="tbl-except">
            <table border="0" cellpadding="0" cellspacing="0">
                ...
                <tr data-group="{{gid}}">
                    <td class="cell-area">
                        <a href="#" class="acc_popup edit J_EditArea" data-acc="event:enter"
                           area-controls="J_DialogArea" area-haspopup="true" title="编辑运送区域">编辑</a>
                        <div class="area-group">
                            {{#isBatch}}<input type="checkbox" name="{{delivery}}_{{gid}}" value="" class="J_BatchField"
                                               {{#checked}}checked="checked" {{/checked}}/>{{/isBatch}}
                            {{#group}}
                            <p>{{str}}</p>
                            {{/group}}
                    ...
                    <td><input type="text" name="{{delivery}}_start_{{gid}}" data-field="start" value="{{start}}"
                               class="input-text {{#starterror}}input-error {{/starterror}}" autocomplete="off"
                               maxlength="6" aria-label="首{{typeName}}"/></td>
                    <td><input type="text" class="j_sellerBearFrePrice" value="0.00" disabled="disabled"/><input
                                type="text" name="{{delivery}}_postage_{{gid}}" data-field="postage" value="{{postage}}"
                                class="input-text {{#postageerror}}input-error {{/postageerror}}" autocomplete="off"
                                maxlength="6" aria-label="首费"/></td>
                    <td><input type="text" name="{{delivery}}_plus_{{gid}}" data-field="plus" value="{{plus}}"
                               class="input-text {{#pluserror}}input-error {{/pluserror}}" autocomplete="off"
                               maxlength="6" aria-label="续{{typeName}}"/></td>
                    <td><input type="text" class="j_sellerBearFrePrice" value="0.00" disabled="disabled"/><input
                                type="text" name="{{delivery}}_postageplus_{{gid}}" data-field="postageplus"
                                value="{{postageplus}}"
                                class="input-text {{#postagepluserror}}input-error {{/postagepluserror}}"
                                autocomplete="off" maxlength="6" aria-label="续费"/></td>
                    {{#global}}
                    <td>
                        <input type="text" class="j_sellerBearFrePrice" value="0.00" disabled="disabled"/>
                        <input type="text" name="{{delivery}}_wayDay_{{gid}}" data-field="wayDay" value="{{wayDay}}"
                               class="input-text J_ItemWayDay {{#wayDayerror}}input-error {{/wayDayerror}}"
                               autocomplete="off" maxlength="6" aria-label="时效"/>
                    </td>
                    {{/global}}
        ...
        <div class="tbl-attach">
            <div class="J_SpecialMessage">{{#specialmessage}}<span class="msg J_Message"><span
                            class="error">{{.}}</span></span>{{/specialmessage}}
            </div>
...
<script id="J_FreeTemplate" type="text/template">
    ...
    <tr data-index="{{xindex}}">
        ...
        <td>
            ...
            <p class="free-contion">
                {{@if designated==="0"}}
                {{act}} <input type="text" value="{{typeValue}}"
                               class="input-text {{@if preferentialStandardError}}input-error{{/if}}" name="{{type}}">
                {{style}}包邮{{/if}}
                {{@if designated==="1"}}
                {{act}} <input type="text" name="preferentialMoney"
                               class="input-text input-65 {{@if preferentialMoneyError}}input-error{{/if}}"
                               value="{{preferentialMoney}}"> 元以上{{/if}}
                {{@if designated==="2"}}
                {{act}} <input type="text" value="{{typeValue}}"
                               class="input-text {{@if preferentialStandardError}}input-error{{/if}}" name="{{type}}">
                {{style}},<input type="text" name="preferentialMoney"
                                 class="input-text input-65 {{@if preferentialMoneyError}}input-error{{/if}}"
                                 value="{{preferentialMoney}}"> 元以上包邮{{/if}}
            </p>
            {{/with}}
        </td>
        <td>
...
<script>
    var presetData = {
        "list":<?=htmlspecialchars_decode($ShipTemplate['sDeliveryJson'])?>,
        "type": "<?=strtolower($ShipTemplate['sValuation'])?>"
    };
    var areaGroup = {
        "group": [{
            "title": "华东",
...
<script>
    ...
    //条件包邮模拟数据
    initJSON = <?=htmlspecialchars_decode($ShipTemplate['sFreeTypeJson'])?>;
    var switchFlag = false
    switchFlag = true
</script>

<script id="J_SpecialTemplate" type="text/template">
    ...
    <div class="tbl-except">
        <table border="0" cellpadding="0" cellspacing="0">
            ...
            <tr data-group="{{gid}}">
                <td class="cell-area">
                    <a href="#" class="acc_popup edit J_EditArea" data-acc="event:enter" area-controls="J_DialogArea"
                       area-haspopup="true" title="编辑运送区域">编辑</a>
                    <div class="area-group">
                        {{#isBatch}}<input type="checkbox" name="{{delivery}}_{{gid}}" value="" class="J_BatchField"
                                           {{#checked}}checked="checked" {{/checked}}/>{{/isBatch}}
                        {{#group}}
                ...
                <td><input type="text" name="{{delivery}}_start_{{gid}}" data-field="start" value="{{start}}"
                           class="input-text {{#starterror}}input-error {{/starterror}}" autocomplete="off"
                           maxlength="6" aria-label="首{{typeName}}"/></td>
                <td><input type="text" class="j_sellerBearFrePrice" value="0.00" disabled="disabled"/><input type="text"
                                                                                                             name="{{delivery}}_postage_{{gid}}"
                                                                                                             data-field="postage"
                                                                                                             value="{{postage}}"
                                                                                                             class="input-text {{#postageerror}}input-error {{/postageerror}}"
                                                                                                             autocomplete="off"
                ...
                <td><input type="text" name="{{delivery}}_plus_{{gid}}" data-field="plus" value="{{plus}}"
                           class="input-text {{#pluserror}}input-error {{/pluserror}}" autocomplete="off" maxlength="6"
                           aria-label="续{{typeName}}"/></td>
                <td><input type="text" class="j_sellerBearFrePrice" value="0.00" disabled="disabled"/><input type="text"
                                                                                                             name="{{delivery}}_postageplus_{{gid}}"
                                                                                                             data-field="postageplus"
                                                                                                             value="{{postageplus}}"
                                                                                                             class="input-text {{#postagepluserror}}input-error {{/postagepluserror}}"
                                                                                                             autocomplete="off"
...
<script id="J_SpecialItemTemplate" type="text/template">
    <tr data-group="">
        <td class="cell-area">
            <a href="#" class="acc_popup edit J_EditArea" data-acc="event:enter" area-controls="J_DialogArea"
               area-haspopup="true" title="编辑运送区域">编辑</a>
            <div class="area-group">未添加地区</div>
            <input type="hidden" name="_areas_" value="">
        </td>
        <td><input type="text" name="{{delivery}}_start_{{gid}}" data-field="start" value="{{start}}"
                   class="input-text {{#starterror}}input-error {{/starterror}}" autocomplete="off" maxlength="6"
                   aria-label="首{{typeName}}"/></td>
        <td><input type="text" class="j_sellerBearFrePrice" value="0.00" disabled="disabled"/><input type="text"
                                                                                                     name="{{delivery}}_postage_{{gid}}"
                                                                                                     data-field="postage"
                                                                                                     value="{{postage}}"
...
<input type="hidden" id="J_AreaGrid" data-url="/js/address_allarea_json.php?callback=AreaGrid.load&version=0"/>
<script src="/js/package.js" charset="utf-8"></script>
...
<script>
    ...
    (function () {
        var getRootDomain = function () {
            var host = arguments[1] || location.hostname, da = host.split('.'), len = da.length,
                deep = arguments[0] || (len < 3 ? 0 : 1);
...
<!-- <script src="/js/mytemplate.js" charset="utf-8"></script> -->
</body>
</html>
