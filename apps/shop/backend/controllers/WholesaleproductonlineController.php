 ...
 * @version v1.0
 */
class WholesaleproductonlineController extends BackendController
{
    public function getHomeTabs()
    {
        ...
        $list['ID'] = '1';
        $list['sName'] = '出售中';
        $list['sKey'] = 'Main.Shop.WholesaleProductonline.List.All';
        $list['sLinkUrl'] = \Yii::$app->homeUrl . '/' . strtolower($this->sObjectName) . '/getlisttable?sListKey=Main.Shop.WholesaleProductonline.List.All&sTabID=onsale&sExtra=onsale';
        $data['arrList'][] = $list;

        $list['ID'] = '2';
        $list['sName'] = '已售罄';
        $list['sKey'] = 'Main.Shop.WholesaleProductonline.List.All';
        $list['sLinkUrl'] = \Yii::$app->homeUrl . '/' . strtolower($this->sObjectName) . '/getlisttable?sListKey=Main.Shop.WholesaleProductonline.List.All&sTabID=saleout&sExtra=saleout';
        $data['arrList'][] = $list;

        $list['ID'] = '3';
        $list['sName'] = '待上架';
        $list['sKey'] = 'Main.Shop.WholesaleProductonline.List.All';
        $list['sLinkUrl'] = \Yii::$app->homeUrl . '/' . strtolower($this->sObjectName) . '/getlisttable?sListKey=Main.Shop.WholesaleProductonline.List.All&sTabID=offsale&sExtra=offsale';
        $data['arrList'][] = $list;
        ...
        $list['sName'] = '所有';
        $list['sKey'] = 'Main.Shop.WholesaleProductonline.List.All';
        $list['sLinkUrl'] = \Yii::$app->homeUrl . '/' . strtolower($this->sObjectName) . '/getlisttable?sListKey=Main.Shop.WholesaleProductonline.List.All&sTabID=all&sExtra=all';
        $data['arrList'][] = $list;
    ...
    {
        $arrConfig['arrFlt'][] = ['sField' => 'bGroup', 'sOper' => 'equal', 'sValue' => '1'];
        if ($_POST['sExtra'] == 'all') {
