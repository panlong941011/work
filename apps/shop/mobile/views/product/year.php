    ...
    <? if ($arrCat) { ?>
        <div class="cat-viewport">
            ...
            <div class="cat-con" id="cat-con">
                <!--初始化第一个加item-cur-->
                <? foreach ($arrCat as $key => $value) { ?>
                    <div class="con-item <? if ($key == $current) {
                        echo 'item-cur';
                    } ?>">
                    </div>
                <? } ?>
...
<footer>
    <div class="bottom_fixed flex">
        ...
        <a href="<?= Url::toRoute([\Yii::$app->request->shopUrl . "/product/year"], true) ?>" class="on">
            <span class="icon">&#xe655;</span>
            <p>年度计划</p>
        </a>
