        ...
        <div class="top_colum">
            ...
            <div class="select_area" v-if="!isEmpty">
                <div class="area_list flex">
                    ...
                    <div class="select_filter flex">
                        <b class="icon">&#xe635;</b> <em>筛选</em>
                    </div>
                            ...
                            <li v-for="item in dataList">
                                <a :href="item.link" class="flex">
                                    <div class="list_img">
                                        ...
                                        <div class="list_kill" v-if="item.seckill.length !== 0"></div>
                                        <div class="list_label" v-if="item.seckill.length == 0" :style="{ backgroundImage:'url('+item.icon+')'}"></div>
                                        <div class="list_img_mask" v-if="item.saleout || item.seckill.status === '已抢光' || item.seckill.status === '已结束'">
                                            <div class="l_wrap">
                                                <img src="/images/home/sellout.png" alt="" v-if="item.saleout">
                                                <img src="/images/home/lootall.png" alt="" class="tip_img" v-if="item.seckill.status === '已抢光'">
                                                <img src="/images/home/over.png" alt="" class="tip_img" v-if="item.seckill.status === '已结束'">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="list_content flexOne">
                                        <h2 class="multiEllipsis" v-text="item.title"></h2>
                                        <spc-list v-if="item.seckill.status == '未开始'" :datatime="item.seckill.start"></spc-list>
                                        <div class="rob_num" v-if="item.seckill.status == '未抢光'" v-text="'仅剩'+item.seckill.stock+'件'"></div>
                                        <div class="rob_status" v-if="item.seckill.status == '已抢光'">已抢光</div>
                                        <div class="rob_status" v-if="item.seckill.status == '已结束'">已结束</div>
                                        <del v-text="item.market_price"></del>
                                        <em v-text="item.price" class="price"></em>
                                        <!--<span v-text="'已售'+item.sold+'件'" class="sold" v-if="item.seckill.length === 0"></span>-->
                                    </div>
                                </a>
                            </li>
                        </ul>
				...
				<? if ($arrTag) { ?>
                    <div class="f_label">
                        ...
                        <div class="f_l_content">
                            <span class="all <?= $_GET['tag'] ? "" : "on" ?>" data-id="">全部</span> <!-- 第一个要特殊处理 class all其他span不能有 -->
							<?
				...
				<? if ($arrBrand) { ?>
                    <div class="f_classify">
                        ...
                        <div class="f_c_content">
                            <span class="all <?= $_GET['brand'] ? "" : "on" ?>" data-id="">全部</span> <!-- 第一个要特殊处理 class all其他span不能有 -->
							<?
...
<?php $this->beginBlock('foot') ?>
    ...
    <script>
            ...
            $('.search_btn').on('click', function () {
                ...
                setTimeout(function (args) {
                    location.href = "<?=\yii\helpers\Url::toRoute([\Yii::$app->request->shopUrl . "/product/list"],
						true)?>?keyword=" + encodeURI(value) + "&bWholesale=" + bWholesale; //跳转模
                }, 500);
        ...
        function toSearch(keyword) {
            ...
            location.href = url + '?keyword=' + keyword + "&bWholesale=" + bWholesale;
            if (isIOS()) {
                $('.search_wrap').hide();
                $('.searchList_wrap').show();
            }
