...
<style>
    ...
    .div_promotion {
        ...
        height: 4.66rem;
        z-index: 4;
        background: url(/images/detail/timerpirce.png) no-repeat;
...
<div class="detail_wrap">
    ...
    <div class="timer" style="background-color: #E0B991">
        <div style="float: left;font-size: 0.9rem;margin-left: 0.5rem;">
            ¥<? $arr = explode('.',$product->fPrice);echo $arr[0]?>.<em style="font-size: .7rem"><? $arr = explode('.',$product->fPrice);echo $arr[1]?></em> <span>限时拼团</span>
        </div>
        <div style="float: right;margin-right:1rem;color: #fff;line-height: 1rem;font-size: 0.5rem;" id="promotionDate">
        </div>
...
<!--        --><?// if ($product->fGroupPrice > $product->fPrice) { ?>
<!--            <div class="sole_price flex">-->
<!--                <em class="s_p_price">原价：¥--><?//= number_format($product->fGroupPrice, 2) ?><!--</em>-->
<!--                <em class="s_p_price" style="font-size: 0.5rem;border: 1px red solid;margin-left: 1rem;line-height: 1rem;border-radius: 0.2rem;padding: 0.1rem">限时--><?//= number_format($product->fPrice/$product->fGroupPrice, 2)*10 ?><!--折</em>-->
<!--            </div>-->
<!--        --><?// } else { ?>
<!--            <div class="sole_price flex">-->
<!--                <em class="s_p_price">团购价：¥--><?//= number_format($product->fGroupPrice, 2) ?><!--</em>-->
<!--            </div>-->
<!--        --><?// } ?>
    ...
    <div class="sales_volume flex" style="line-height: 1rem;height: 1.3rem;padding-left: 0.5rem;padding-right: 0.5rem;padding-bottom: 0.2rem">
        <span class="stock" style="font-size: 0.65rem">运费&nbsp;免运费</span>
        <span class="stock" style="font-size: 0.65rem">库存&nbsp;<?= intval($product['lStock']) ?></span>
<!--        <span class="stock" style="font-size: 0.65rem">销量&nbsp--><?//= intval($product['lSale']) ?><!--</span>-->
    ...
    <footer>
        <div class="fix_btn flex">
            <a href="<?= Url::toRoute([\Yii::$app->request->shopUrl . "/home/index"], true) ?>" style="display:block;width: 20%;font-size: .5rem;text-align: center;">
                <img src="/images/detail/home.png" alt="" style="width:1.1rem;height: 1.1rem;margin: auto;margin-top:.1rem">
                首页
            </a>

            <a href="<?= Url::toRoute([\Yii::$app->request->shopUrl . "/cart/index"], true) ?>" style="display:block;width: 20%;font-size: .5rem;text-align: center;">
                <img src="/images/detail/cart.png" alt="" style="width:1.1rem;height: 1.1rem;margin: auto;margin-top:.1rem">
                购物车
            </a>

            <div class="add_car" style="width: 30%">加入购物车</div>
            <div id="buy_now" style="width: 30%;background: #E0B991"
                 class="buy_now <? if (!$product->bOnSale ||$timerMsg=='距开始仅剩') { ?>buy_off<? } ?>">
                <?if($timerMsg=='距开始仅剩'){?>
                    即将开抢
                <?}else{?>
                    立即购买
    ...
    <div class="chose_item">
        <div class="sample flex">
            ...
            <div class="s_cont">
                ...
                <em>¥<i id="price" style="font-weight:700"><?= $product->fGroupPrice ?></i></em>
            </div>
        ...
        <div class="sure_btn" style="background: #E0B991">确定</div>
    </div>
</section>
...
<script>
    ...
    $(function () {
        ...
        $('.sure_btn').on('click', function () { //规格确定按钮
            ...
            if (isBtn == 'car') {
                $.post('<?= Url::toRoute(["/cart/addtocart"], true) ?>',
                    {
                        productid: pId,
                        quantity: number,
                        group: 'group',
            ...
            //立即购买
            if (isBtn == 'buy') {
                $.post('<?= \yii\helpers\Url::toRoute(["/cart/addgroupcart"], true) ?>',
                    {
                        productid: pId,
                        quantity: number,
                        group: 'group',
...
<script>
    ...
    function promotionToEnd(nowTime, endTime, timerMsg) {
        var t = setInterval(function () {
            ...
            $('#promotionDate').html('<p>' + timerMsg + '</p>' + hours + '&nbsp;时&nbsp;' + minutes + '&nbsp;分&nbsp;' + seconds + '&nbsp;秒&nbsp;');
        }, 1000);
    }
