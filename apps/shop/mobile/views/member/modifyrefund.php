...
<?php $this->endBlock() ?>
    <div class="refund_apply" id="app" v-cloak>
        <div class="ad_header">
            ...
            <h2>修改申请</h2>
            <span class="ad_more" onclick="location.href='<?= \yii\helpers\Url::toRoute([
                "/member/refundpolicy"
            ], true) ?>';">退款政策</span>
        </div>
        <section>
                ...
                <div class="apply_price flex">
                    ...
                    <div class="item_content">
                        <input type="text" name="" id="totalnum" class="price"
                               value="<?= $refund->lItemTotal ?>" <? if ($unship) {
                            echo 'readonly';
            ...
            <div class="apply_explain">
                ...
                <div class="item_content">
                    <textarea class="explain_word" maxlength="150" placeholder="最多150字"
                              v-model="refundExplain"><?= $refund->sExplain ?></textarea>
                </div>
            </div>
...
<?php $this->beginBlock('foot') ?>
    ...
    <script>
        ...
        new Vue({
            ...
            data: {
                refundType: '<?=$refund->sType?>', //退款类型
                refundReason: '<?=$refund->sReason?>',//退款原因
                refundPrice: '<?=number_format($refund->fRefundApply, 2, ".", "")?>',//退款金额
                maxRefundPrice: '<?=number_format($fRefundMoney, 2, ".", "")?>',//最大退款金额
                refundExplain: '<?=$refund->sExplain?>',//退款说明
                imgList: <?=$imgList?>, //存储图片 传给后端的图片数据
                totalNum: '<?=$refund->lItemTotal?>', //商品总数量
                refundNum: '<?=$refund->lRefundItem?>',//退款数量
            ...
            methods: {
                ...
                submitData: function () {
                    ...
                    /* if (_this.refundExplain == '') {
                     shoperm.showTip('请填写退款说明');
                     return;
                     }*/
                    if (_this.totalNum == '') {
                    ...
                    $.post('/member/modifyrefund?id=<?=$_GET['id']?>',
                        {
