    ...
    <div class="ad_header">
        ...
        <div class="withdraw_main">
            ...
            <div class="withdraw_amount">
                <span>提现金额</span>
                <input type="" name="" placeholder="余额:¥<?= number_format($seller->computeWithdraw, 2)  ?>" class="amount"
                       id="sAmount">
            </div>
        </div>
        <div class="withdraw_tip">
            <p>金额低于 <em><?= \myerm\shop\common\models\MallConfig::getValueByKey('lWithdrawMin') ?></em> 元时不可提现</p>
    ...
    $(function () {
        ...
        $('.withdraw_btn').on('click', function () {
            ...
            if (price < lWithdrawMin) {
                shoperm.showTip("金额低于1元时不可提现");
                return false;
            }

            if (price > <?=$seller->computeWithdraw ?>) {
            ...
            $.post(
                ...
                function (data) {
                    ...
                    if (data.status) {
                        ...
                        setTimeout("location.href='/seller';", 1000);
                    } else {
