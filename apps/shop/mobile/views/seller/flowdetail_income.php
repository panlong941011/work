...
<div class="flow_details">
    ...
    <div class="person">
        <div class="buyer flex">
            <div class="logo_pic">
                <img src="<?=$flow->order->member->avatar?>">
            </div>
            <div class="person_info">
                <h3>买家</h3>
                <p><?=$flow->order->member->sName?></p>
            </div>
        </div>
        <div class="suppiler flex">
            <div class="logo_pic">
                <img src="<?=$flow->sellerOrder->seller && $flow->sellerOrder->seller->member ? $flow->sellerOrder->seller->member->avatar : "/images/order/person.png"?>">
            </div>
            <div class="person_info">
                <h3>经销商</h3>
                <p><?=$flow->sellerOrder->seller->sName?></p>
            </div>
        </div>
    </div>
    <div class="buy_commity">
        <h2>购买商品</h2>
        <ul onclick="location.href='/member/order?id=<?=$flow->OrderID?>'">
            <? foreach ($flow->arrOrderDetail as $detail) { ?>
            <li class="b_c_item">
                <p><?=$detail->sName?></p>
                <span><?=$flow->order->dNewDate?></span>
            </li>
            <? } ?>
        </ul>
    </div>
