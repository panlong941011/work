                    ...
                    <? foreach ($arrAddress as $address) { ?>
                        <li data-id="<?= $address->lID ?>">
                            <div class="address_item chose_item">
                                ...
                                <? if ($_GET['addressid'] == $address->lID || !$_GET['addressid'] && $address->bDefault) { ?>
                            ...
                            <div class="address_ops flex">
                                <div class="address_set">
                                    <span
                                        class="address_set_btn <? if ($address->bDefault) { ?>default<? } ?>">已设为默认</span>
                                </div>
    ...
    <div class="add_mask" style="display: none;">
        <div class="addr_main">
            ...
            <div class="addr_region">
                <input type="text" readonly id="J_Address" style="cursor: pointer" class="c_area flexOne" name="input_area" placeholder="请选择地区">
            </div>
    ...
    <script>
        ...
        $(function () {
                ...
                if (areaType == 'new') {
                    ...
                    $.post('/address/new', areaData,
                        function (res) {
                            ...
                            if (res.status) {
                                <? if($sFrom == 'SellerJoin'){ ?>
                                location.href = "<?= \yii\helpers\Url::toRoute(["/seller/join"], true) ?>?addressid=" + res.addressid ;
                                <?}else{?>
                                    <?if(strpos($_SERVER['REQUEST_URI'],'shop01')){?>
                                        location.href = "<?= \yii\helpers\Url::toRoute(["/shop01/cart/checkout"], true) ?>?addressid=" + res.addressid;
                                    <?}else{?>
                ...
                if (areaType == 'edit') {
                    ...
                    $.post('/address/edit', areaData,
                        function (res) {
                            ...
                            if (res.status) {
                                <? if($sFrom == 'SellerJoin'){ ?>
                                location.href = "<?= \yii\helpers\Url::toRoute(["/seller/join"], true) ?>?addressid=" + editId ;
                                <?}else{?>
                                    <?if(strpos($_SERVER['REQUEST_URI'],'shop01')){?>
                                    location.href = "<?= \yii\helpers\Url::toRoute(["/shop01/cart/checkout"], true) ?>?addressid=" + editId;
                                    <?}else{?>
            ...
            $('.address_item').on('click', function () {
                ...
                console.log(id);
                <? if($sFrom == 'SellerJoin'){ ?>
                location.href = "<?= \yii\helpers\Url::toRoute(["/seller/join"], true) ?>?addressid=" + id ;
                <?}else{?>
                    <?if(strpos($_SERVER['REQUEST_URI'],'shop01')){?>
                    location.href = "<?= \yii\helpers\Url::toRoute(["/shop01/cart/checkout"], true) ?>?addressid=" + id;
                    <?}else{?>
