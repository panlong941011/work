...
<?php $this->beginBlock('style') ?>
    ...
    <style>
        ...
        .commodity {
            ...
            width: 100%;
            clear: both;
            background-color: rgba(229, 229, 229, 0.5714285714285715);
            height: 3.5rem;
        }
...
<?php $this->endBlock() ?>
    ...
    <div class="index_wrap" id="app1" v-cloak>
        <!-- 搜索框-->
        <div style="width: 100%;height: auto;clear: both;background-color: #fff;padding-top:20px;">
            <div class="top_search flex" id="search"><span class="mirror"></span> <span class="word">搜索商品</span></div>
        </div>

        <!-- 轮播图结构 -->
        <ul class="lsj_tab_list">
            <li class="<?=$_GET['Type']?'':'cur'?>">
                <span><a href="<?= Url::toRoute([\Yii::$app->request->shopUrl . "/material/index"], true) ?>">全部动态</a></span>
            </li>
            <li class="<?=$_GET['Type']?'cur':''?>">
                <span><a href="<?= Url::toRoute([\Yii::$app->request->shopUrl . "/material/index?Type=1"], true) ?>">我发布的</a></span>
            </li>
        </ul>
        <!--   商品结构 -->
        <div class="data_wrap">
                    ...
                    <div class="commodity_list">
                        ...
                        <ul>
                            <li v-for="(lItem,index) in item.commodity">
                                <div v-text="lItem.dNewDate" class="commodity_date"></div>
                                <div v-html="lItem.sContent" class="commodity_txt"></div>
                                <div   v-if="lItem.detail != '-1'" style="clear: both;width: 100%;margin-top: 0.3rem;min-height: 4rem;padding-left: 1rem; ">
                                    <div v-for="(img,index) in lItem.detail" class="commodity_img">
                                        <img src="/images/home/list.png" :x-src="img" alt=""
                                             class="lazy">
                                    </div>
                                </div>
                                <div style="height: 4rem;clear: both;">
        ...
        new Vue({
            ...
            methods: {
                upCallback: function () {
                    ...
                    $.ajax({
                        type: 'GET',
                        url: '<?=Url::toRoute([\Yii::$app->request->shopUrl . "/material/item?Type"+Type], true)?>',
                        data: {
                            index: _self.page,
                            type: 2
                        },
    ...
    <script>
        $(function () {
            ...
            $('.backTop').on('click', function () {
                location.href = '<?= Url::toRoute([\Yii::$app->request->shopUrl . "/material/edit"], true)?>';
            })
            //搜索栏效果
