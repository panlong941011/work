            ...
            <div class="cat-nav" id="cat-nav">
                <ul>
					...
					<? foreach ($arrCat as $key => $value) { ?>
                        <?if($value['lID']==1){?>
                            <li catid=<?= $value['lID'] ?>  bRecommend="1" class="cur"><span class="color: rgba(212, 48, 48, 1);"><?= $value['sName'] ?></span></li>
                        <?}else{?>
                        <li catid=<?= $value['lID'] ?>><?= $value['sName'] ?></li>
                        <?}?>
					<? } ?>
                </ul>
            </div>
            <div class="cat-con" id="cat-con">
                <div class="agentrecommendword">
                    <p>店长推荐最多展示10个商品</p>
                    <p>快挑选中意的商品到这里吧！</p>
                </div>
                <!--初始化第一个加item-cur-->
