...
<div class="detail_wrap">
	...
	<div class="banner_swiper">
		<div class="banner_label" style="background-image: url('<?=$product->icon?>');"></div>
		<? if ($product->pics) { ?>
	...
	<div class="describe">
		...
		<div class="sole_price flex">
			<em class="s_p_price"><?= $wholsale->showSalePrice ? '¥' . $wholsale->showSalePrice : '' ?></em>
		</div>
		<del class="old_price"><?= $wholsale->showPrice ? '¥' . $wholsale->showPrice : '' ?></del>
		<? if (Yii::$app->frontsession->seller) {?>
			<div class="detail_prompt flex">
				<i class="icon">&#xe73a;</i>
				<div class="prompt_word flex">
					<p>消费 </p><em>
						省<?= number_format($product['fProfit'] * Yii::$app->frontsession->seller->type->lSaleRate / 100,
							2) ?>元，</em>
					<p>分享 </p><em>
						赚<?= number_format($product['fProfit'] * Yii::$app->frontsession->seller->type->lSaleRate / 100,
							2) ?>元</em>
				</div>
			</div>
		<? } ?>
		<div class="sales_volume flex">
			...
			<span clsas="sold">已售<?= intval($wholsale['lSale']) ?></span>
		</div>
	...
	<div class="guarantee flex">
		<div class="guarantee_item">
			<i class="icon">&#xe626;</i>
			<span class="guarantee_name">正品保证</span>
		</div>
		<div class="guarantee_item">
			<i class="icon">&#xe626;</i>
			<span class="guarantee_name">包邮包税</span>
		</div>
		<div class="guarantee_item">
			<i class="icon">&#xe626;</i>
			<span class="guarantee_name">售后无忧</span>
		</div>
	</div>
	<div class="line"></div>
	<!--运费-->
	<div class="transport">
		<div class="transto flex">
			<span>送至</span>
			<input type="text" readonly id="J_Address" class="c_area flexOne" name="input_area" placeholder="请选择收货地址">
		</div>
	...
	<!--供应商-->
	<div class="supplier">
		<a href="<?= Url::toRoute([\Yii::$app->request->shopUrl . "/supplier/detail", 'id' => $product->supplier->lID],
			true) ?>"
			class="flex">
			<div class="sup_pic">
	...
	<footer>
		<div class="fix_btn flex">
			<div class="car">
				<a href="<?= \yii\helpers\Url::toRoute(["/cart"], true) ?>" class="flex"> <img
						src="/images/detail/car.png" alt=""> <span>购物车</span> </a>
				<? if ($lCartProductQty) { ?>
					<span class="add_num">
                        <i class="flex"><?= $lCartProductQty >= 100 ? '•••' : $lCartProductQty ?></i>
                    </span>
				<? } else { ?>
					<span class="add_num" style="display: none;">
                        <i class="flex"></i>
                    </span>
				<? } ?>
			</div>
			<div class="service">
				<a href="tel:<?= \myerm\shop\common\models\MallConfig::getValueByKey("sServiceNum") ?>" class="flex">
					<img src="/images/detail/kefu.png" alt=""> <span>客服</span> </a>
			</div>
			<div class="add_car <? if (!$product->bOnSale) { ?>add_off<? } ?>">
				加入购物车
			</div>
			<div class="buy_now <? if (!$product->bOnSale) { ?>buy_off<? } ?>">
				立即购买
			</div>
		</div>
	</footer>
	...
	<div class="chose_item">
		<div class="sample flex">
			...
			<div class="s_cont">
				...
				<em>¥<i id="price" style="font-weight:700"><?= $wholsale->showSalePrice ?></i></em> <span
					id="tip">请选择规格</span>
			</div>
...
<div class="ewm_wrap" id="ewmWrap">
	<div class="ewm_header flex">
		<div class="header_pic"
			style="background-image: url(<?= \Yii::$app->request->imgUrl ?>/<?= \myerm\shop\common\models\MallConfig::getValueByKey("sMallLogo") ?>)">
			<!--  <img src=""> -->
		</div>
	...
	<div class="ewm_tip">长按保存，分享给你的朋友</div>
